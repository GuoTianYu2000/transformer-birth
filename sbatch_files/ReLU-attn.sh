...
# only ATTN in the first layer, and only MLP in the last layer
    python ihead_full_main.py max_iters=${iter_num} log_probes=True eval_delta=5  task_name=dormant_copy fine_grid_log=${iter_num} seperate_loss=True seed=42 device_num=2\
    ...
    wandb_args.name=dormant_copy_ReLU_attn\
    optim_args.use_sgd=False optim_args.learning_rate=0.003 optim_args.weight_decay=1e-4 optim_args.batch_size=512 \
    save_dir=/data/tianyu/birth/gens/pre-iclr/dynamics/ReLU_attn num_data_workers=60
